...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::diablo)
    {
    }
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizediabloAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizediabloAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizediabloAmount = strAnonymizediabloAmount.remove(strAnonymizediabloAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizediabloAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizediabloAmount * COIN) nMaxToAnonymize = nAnonymizediabloAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizediabloAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizediabloAmount));
        strAnonymizediabloAmount = strAnonymizediabloAmount.remove(strAnonymizediabloAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizediabloAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
