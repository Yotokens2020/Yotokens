...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::yotokens)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("yotokens")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeyotokensAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeyotokensAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeyotokensAmount = strAnonymizeyotokensAmount.remove(strAnonymizeyotokensAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeyotokensAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeyotokensAmount * COIN) nMaxToAnonymize = nAnonymizeyotokensAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeyotokensAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeyotokensAmount));
        strAnonymizeyotokensAmount = strAnonymizeyotokensAmount.remove(strAnonymizeyotokensAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeyotokensAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
